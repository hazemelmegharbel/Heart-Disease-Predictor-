...
%Extracting the data (age, sex, bp, etc..) of the healthy 
%patients and the sick ones separately to then later be 
%able to extract the covariance matrix for each class.
...
%calculates the new variable y=v1*f1+v2*f2...v13*f13 
%for the healthy  patients. The fis are the age,
%sex, etc..
Hvars=[];
for i= 1:160
...
%% 
%calculates the new variable y=v1*f1+v2*f2...v13*f13 
%for the ill  patients. The fis are the age,
%sex, etc..
Svars=[];
...
%In this section we form the 2 Gaussians
%to later be able to select the most effective threshold.
x_vals=-10:0.001:10; 

%Gaussian for non-hypertensive (healthy) patients
pd1 = fitdist(Hvars','Normal');

%Gaussian for hypertensive (ill) patients.
pd2 = fitdist(Svars','Normal');
